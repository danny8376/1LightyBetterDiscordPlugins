...
https://astranika.com/bd/download?plugin=1XenoLib`, {
  ...
  initialize() {
    ...
    if (this.settings.dontSaveData) {
      ...
      if (data && data.messageRecord === 'Split') data.messageRecord = this.loadSplitMessageRecords(this.getName() + 'DataMessageRecord');
      ...
      if (isBad(data)) {
        if (this.settings.autoBackup) {
          ...
          if (data && data.messageRecord === 'Split') data.messageRecord = this.loadSplitMessageRecords(this.getName() + 'DataMessageRecordBackup');
          if (isBad(data)) {
...
*/
  ...
  handleDataSaving() {
    ...
    if (useEfficient) {
      if (Object.keys(this.messageRecord).length > 5000) this.efficientlySaveData(
        this.getName() + 'Data',
        'data',
        {
          messageRecord: 'Split',
        ...
        err => {
          if (err) callback(err);
          else this.saveSplitMessageRecords(this.getName() + 'DataMessageRecord', callback);
        }
      );
      else this.efficientlySaveData(
        this.getName() + 'Data',
        'data',
        {
  ...
  efficientlySaveData(name, key, data, callback) {
    try {
      ...
      ((obj2stringify, asyncStringifyCallback) => {
        // from: https://github.com/ibmruntimes/yieldable-json/blob/5809572f1957a76e6c6137ca2b9fd5640b96eb0d/yieldable-stringify.js
        let counter = 0;
        let objStack = [];
        let temp = '';
        const limit = 100000;
        ...
        let stringifyWrapper = (value, replacer, space, intensity, callback) => {
          ...
          function stringifyAsync(data, replacer, space, intensity, callback) {
          ...
          stringifyAsync(obj2stringify, asyncStringifyCallback);
      })({ [key]: data }, async (err, json) => {
          if (err) ZeresPluginLibrary.Logger.err(this.getName(), 'There has been an error stringifying data for saving to the data file', err);
          else {
      async function asyncWrite(stream, data) {
        const chunkSize = 1024*1024; // 1MiB chunk
        for (let i=0; i < data.length; i += chunkSize) await new Promise(resolve => {
          setTimeout(() => {
            stream.write(data.substr(i, chunkSize));
            resolve();
          }, 0);
        });
      }
      try {
        let stream = this.nodeModules.fs.createWriteStream(this.__isPowerCord ? BdApi.__getPluginConfigPath(name) : this.nodeModules.path.join(this.pluginDir, `${name}.config.json`));
        await asyncWrite(stream, json);
        stream.end();
        callback();
      } catch (e) {
        callback(e);
  ...
  loadSplitMessageRecords(name) {
    // this.__isPowerCord not init yet
    let __isPowerCord = !!window.powercord && typeof BdApi.__getPluginConfigPath === 'function' || typeof global.isTab !== 'undefined';
    if (__isPowerCord) return XenoLib.loadData(`${name}.NoSplit`, 'split', {}, true);
    else {
      const messageRecord = {};
      for (const filename of this.nodeModules.fs.readdirSync(this.nodeModules.path.join(this.pluginDir, name))) {
        if (!filename.endsWith(".config.json")) continue;
        console.log(filename);
        const split = JSON.parse(this.nodeModules.fs.readFileSync(this.nodeModules.path.join(this.pluginDir, name, filename))).split;
        for (const messageId in split) messageRecord[messageId] = split[messageId];
      }
      return messageRecord;
    }
  }
  // this can cause desynced messagerecords, but i believe it's handled... hope so
  async saveSplitMessageRecords(name, callback) {
    if (this.__isPowerCord) this.efficientlySaveData(`${name}.NoSplit`, 'split', split, callback);
    else try {
      const path = this.nodeModules.path.join(this.pluginDir, name);
      if (this.nodeModules.fs.existsSync(path)) this.nodeModules.fs.rmdirSync(path, { recursive: true });
      this.nodeModules.fs.mkdirSync(path);
      let count = 0;
      let split = {};
      for (const messageId in this.messageRecord) {
        count++;
        split[messageId] = this.messageRecord[messageId];
        const splitSize = 1000; // 10000;
        if (count % splitSize == 0) {
          await new Promise((resolve, reject) => {
            setTimeout(() => {
              //this.efficientlySaveData(this.nodeModules.path.join(name, `Split.${Math.floor(count / splitSize)}`), 'split', split, err => {
              //  if (err) reject(err);
              //  else resolve();
              //});
              try {
                this.nodeModules.fs.writeFileSync(this.nodeModules.path.join(this.pluginDir, name, `Split.${Math.floor(count / splitSize)}.config.json`), JSON.stringify({ split }));
                resolve();
              } catch (e) {
                reject(e);
              }
  ...
  saveBackup() {
    ...
    if (useEfficient) {
      if (Object.keys(this.messageRecord).length > 5000) this.efficientlySaveData(
        ...
        err => {
          if (err) callback(err);
          else this.saveSplitMessageRecords(this.getName() + 'DataMessageRecordBackup', callback);
        }
      );
      else this.efficientlySaveData(
        this.getName() + 'DataBackup',
